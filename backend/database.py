...
class Node(Base):
    ...
    host_info_updated_at = Column(DateTime, nullable=True)
    ...
    # Cached VMs
    vms = relationship("VirtualMachine", back_populates="node", cascade="all, delete-orphan")


class VirtualMachine(Base):
    """Cache delle VM presenti sul nodo (aggiornato asincronamente)"""
    ...
    type = Column(String(10), default="qemu") # qemu, lxc
    status = Column(String(20), default="stopped") # running, stopped, etc.
    
    # Extra info cached
    memory = Column(BigInteger, nullable=True) # Bytes
...
class SyncJob(Base):
    ...
    dest_subfolder = Column(String(100), nullable=True)  # Sottocartella ZFS di destinazione (opzionale)
