...
"""
    ...
    async def run_sanoid(
        ...
        """Esegue sanoid manualmente per creare uno snapshot immediato"""
        # Use --take-snapshots to trigger immediate snapshot creation
        # Adding --force-update to bypass time-based checks if recent snapshot exists
        cmd = "sanoid --take-snapshots --force-update --verbose"
        result = await ssh_service.execute(
            hostname=hostname,
            command=cmd,
            port=port,
    ...
    async def list_snapshots(
        ...
        # Removed grep filter to show ALL snapshots (including manual ones like 'aaa')
        cmd = f"zfs list -t snapshot -o name,creation,used -s creation {dataset} 2>/dev/null || true"
        result = await ssh_service.execute(
            hostname=hostname,
            command=cmd,
            port=port,
