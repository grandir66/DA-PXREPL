...
class SSHKeyService:
    ...
    DEFAULT_AUTHORIZED_KEYS = str(Path.home() / ".ssh" / "authorized_keys") 
    
    def __init__(self):
        # Override se siamo root
        if os.geteuid() == 0:
            self.DEFAULT_KEY_PATH = "/root/.ssh/id_rsa"
            self.DEFAULT_AUTHORIZED_KEYS = "/root/.ssh/authorized_keys"
        pass
    ...
    async def setup_mesh_ssh(
        ...
        Inoltre aggiunge le host key di tutti i nodi ai known_hosts di ogni nodo.
        ...
        # Collect all host IPs for known_hosts distribution
        all_hosts = [node.get('host') or node.get('ip') for node in nodes]
        ...
        for node in nodes:
            ...
            # Step 3: Add all other nodes' host keys to this node's known_hosts
            known_hosts_success = True
            known_hosts_msg = ""
            try:
                other_hosts = [h for h in all_hosts if h != hostname]
                if other_hosts:
                    kh_success, kh_msg = await self.add_hosts_to_known_hosts(
                        target_hostname=hostname,
                        target_port=port,
                        target_username=username,
                        hosts_to_add=other_hosts,
                        key_path=key_path
                    )
                    known_hosts_success = kh_success
                    known_hosts_msg = kh_msg
            except Exception as e:
                known_hosts_success = False
                known_hosts_msg = str(e)
            ...
            results.append({
                ...
                "known_hosts_updated": known_hosts_success,
                "known_hosts_message": known_hosts_msg,
                "success": copy_success and dist_result.success and known_hosts_success
            })
        
        return results
    
    async def add_hosts_to_known_hosts(
        self,
        target_hostname: str,
        ...
        """
        Adds the SSH host keys of specified hosts to the known_hosts file on a target node.
        This allows the target node to connect to other hosts without host key verification prompts.
        """
        key_path = key_path or self.DEFAULT_KEY_PATH
        
        def _add_known_hosts():
            client = paramiko.SSHClient()
            client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ...
            try:
                ...
                for host in hosts_to_add:
                    ...
                    cmd = f"ssh-keyscan -H {host} 2>/dev/null >> ~/.ssh/known_hosts"
                    stdin, stdout, stderr = client.exec_command(cmd)
                    exit_code = stdout.channel.recv_exit_status()
                ...
                client.exec_command("sort -u ~/.ssh/known_hosts -o ~/.ssh/known_hosts")
                
                if failed_hosts:
                    return False, f"Added {len(added_hosts)}, failed {len(failed_hosts)}: {failed_hosts}"
                return True, f"Added host keys for {len(added_hosts)} hosts"
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, _add_known_hosts)
