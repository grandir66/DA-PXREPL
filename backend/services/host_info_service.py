...
class HostInfoService:
    ...
    async def fetch_host_details(
        ...
        Supporta sia PVE (via pvesh) che PBS/Linux generico.
        """
        result = {
            "hostname": hostname,
        ...
        if node_type == "pve":
            ...
            # Fallback per PBS/Linux
            node_info = await self._get_node_info_linux(hostname, port, username, key_path)
            if node_info:
                result.update(node_info)
        ...
        # Storage info
        if include_storage:
            storage_list = await self._get_storage_details(hostname, port, username, key_path, node_type=node_type)
            if storage_list:
    ...
    async def get_host_details(
        ...
        """
        Recupera dettagli host:
        - Se force_refresh=True, scarica nuovi dati e aggiorna DB
        - Altrimenti, restituisce dati da cache DB se presenti
        """
        db = SessionLocal()
        try:
            ...
            # Se cache valida e non forzato, usa cache
            if not force_refresh and node.host_info:
                return node.host_info
            
            # Altrimenti aggiorna
            return await self.update_host_details(node.id)
            
        finally:
            db.close()

    async def update_host_details(self, node_id: int) -> Optional[Dict[str, Any]]:
        """Scarica dati aggiornati e salva nel DB"""
        db = SessionLocal()
        try:
            node = db.query(Node).filter(Node.id == node_id).first()
            if not node:
            ...
            logger.info(f"Aggiornamento dati host per nodo {node.name}...")
            
            details = await self.fetch_host_details(
            ...
            if details:
                node.host_info = details
                node.host_info_updated_at = datetime.utcnow()
                db.commit()
                logger.info(f"Dati host aggiornati per {node.name}")
                return details
            else:
                logger.warning(f"Nessun dato recuperato per {node.name}")
                return None
                
        except Exception as e:
            logger.error(f"Errore aggiornamento host_info per nodo {node_id}: {e}")
            return None
        finally:
            db.close()
    ...
    async def _get_node_info_linux(
        self,
        hostname: str,
        port: int,
        username: str,
        ...
        """Ottiene info base del nodo via comandi Linux standard"""
        try:
            # Uptime
            result = await ssh_service.execute(
                hostname, "cat /proc/uptime | awk '{print $1}'", port, username, key_path
            )
            uptime = int(float(result.stdout.strip())) if result.success and result.stdout.strip() else 0
            
            # Kernel
            result = await ssh_service.execute(
                hostname, "uname -r", port, username, key_path
            )
            kernel = result.stdout.strip() if result.success else "unknown"
            
            # OS Release / Version
            proxmox_version = "Unknown"
            result = await ssh_service.execute(
                hostname, "proxmox-backup-manager version 2>/dev/null", port, username, key_path
            )
            if result.success and result.stdout:
                 proxmox_version = result.stdout.strip().split('\n')[0]
            else:
                 # Try generic debian version
                 result = await ssh_service.execute(
                    hostname, "cat /etc/debian_version", port, username, key_path
                )
                 if result.success:
                     proxmox_version = f"Debian {result.stdout.strip()}"

            return {
                "node_name": hostname, # PBS doesn't have pvesh to get node name distinct from hostname reliably without config
                "proxmox_version": proxmox_version,
                "kernel_version": kernel,
                "uptime_seconds": uptime
            }
        except Exception as e:
            logger.error(f"Errore recupero info nodo linux su {hostname}: {e}")
            return None
    ...
    async def _get_storage_details(
        ...
        """Ottiene informazioni storage (ZFS, LVM, Directory, PBS Datastore)"""
        try:
            # Se è PBS, usa proxmox-backup-manager
            if node_type == "pbs":
                return await self._get_pbs_storage_details(hostname, port, username, key_path)

            # Prima ottieni configurazione storage per flag shared (Solo PVE)
            storage_config = {}
    ...
    async def _get_pbs_storage_details(
        self,
        hostname: str,
        port: int,
        username: str,
        ...
        """Ottiene dettagli datastore PBS"""
        try:
            ...
            if result.success and result.stdout.strip():
                try:
                    ...
                    for ds in datastores:
                         # PBS JSON: {"name": "store1", "path": "...", "used": ..., "total": ...}
                         # Valori in bytes
                         total = int(ds.get("total", 0))
                         used = int(ds.get("used", 0))
                         avail = int(ds.get("avail", 0))
                         
                         store_info = {
                            "name": ds.get("name"),
                            "type": "pbs-datastore",
                            "status": "active", # Assumed active if listed
                         ...
                         if store_info["total_gb"] > 0:
                             store_info["used_percent"] = round((store_info["used_gb"] / store_info["total_gb"]) * 100, 2)
                             
                         storage_details.append(store_info)
                except json.JSONDecodeError:
                    pass
            
            return storage_details
        except Exception as e:
            logger.error(f"Errore raccolta storage PBS: {e}")
            return []
