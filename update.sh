...
if ! command -v rsync >/dev/null 2>&1 || ! command -v sshpass >/dev/null 2>&1 || ! command -v rclone >/dev/null 2>&1; then
    log_info "Installazione rsync/sshpass/rclone per replica file..."
    apt-get update -qq >/dev/null 2>&1 || true
    if DEBIAN_FRONTEND=noninteractive apt-get install -y -qq rsync openssh-client sshpass cifs-utils smbclient rclone >/dev/null 2>&1; then
        log_success "Dipendenze replica file installate"
    else
        log_warn "Impossibile installare dipendenze replica file — eseguire: apt install rsync openssh-client sshpass rclone"
    fi
fi
