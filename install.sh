...
copy_application_files() {
    ...
    if [[ "$UPGRADE_MODE" == true ]]; then
        if [[ -f "$DATA_DIR/dapx.db" ]]; then
            cp "$DATA_DIR/dapx.db" "$DATA_DIR/dapx.db.backup-$(date +%Y%m%d%H%M%S)"
...
generate_secret_key() {
    ...
    cat > "$CONFIG_DIR/dapx-unified.env" << ENV_FILE
# DAPX Unified Configuration
# Generated: $(date)
...
SYSTEMD_UNIT
    ...
    # Logrotate
    cat > /etc/logrotate.d/dapx-unified << LOGROTATE_CONF
$LOG_DIR/*.log {
    daily
    missingok
    rotate 14
...
migrate_database() {
    ...
    local DB_FILE="$DATA_DIR/dapx.db"
...
start_service() {
    ...
    while [[ $retries -gt 0 ]]; do
        if systemctl is-active --quiet dapx-unified; then
    ...
    echo -e "Controlla i log con: ${YELLOW}journalctl -u dapx-unified -n 50${NC}"
    return 1
}
